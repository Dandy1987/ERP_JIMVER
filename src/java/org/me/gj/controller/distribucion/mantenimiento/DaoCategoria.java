...
public class DaoCategoria {
    ...
    private static final Logger LOGGER = Logger.getLogger(DaoCategoria.class);
    ...
    public String insertarCategoria(Categoria objCategoria) throws SQLException {
        ...
        String s_tabsubdir = objCategoria.getTab_subdir();
        String s_tabnomrep = objCategoria.getTab_nomrep();
        int i_tabord = objCategoria.getTab_ord();
        i_flagErrorBD = 0;
        s_msg = "";
        cst = null;
        String SQL_INSERTAR_CATEGORIA = "{call pack_ttabgen.p_033_insertarCategTrans(?,?,?,?,?,?,?)}";
        try {
            ...
            cst.setString(5, objCategoria.getTab_usuadd());
    ...
    public String actualizarCategoria(Categoria objCategoria) throws SQLException {
        ...
        s_msg = "";
        cst = null;
        String SQL_ACTUALIZAR_CATEGORIA = "{call pack_ttabgen.p_033_actualizarCategTrans(?,?,?,?,?,?,?,?,?)}";
        try {
            con = new ConectaBD().conectar();
    ...
    public String eliminarCategoria(Categoria objCategoria) throws SQLException {
        ...
        s_msg = "";
        cst = null;
        String SQL_ELIMINAR_CATEGORIA = "{call pack_ttabgen.p_033_eliminarCategTrans(?,?,?,?)}";
        try {
            con = new ConectaBD().conectar();
    ...
    public ListModelList<Categoria> listaCategoria(int i_caso) throws SQLException {
        String SQL_CATEGORIA;
        if (i_caso == 0) {
            SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_ord,p.tab_nomrep,p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p where p.tab_cod=33 and p.tab_est<>" + i_caso + " order by p.tab_id";
        } else {
            SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_ord,p.tab_nomrep,p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p where p.tab_cod=33 and p.tab_est=" + i_caso + " order by p.tab_id";
        }
        P_WHERE = SQL_CATEGORIA;
        try {
            con = new ConectaBD().conectar();
            st = con.createStatement();
            rs = st.executeQuery(SQL_CATEGORIA);
    ...
    public ListModelList<Categoria> busquedaCategorias(int seleccion, String consulta, int i_estado) throws SQLException {
        String SQL_CATEGORIA;
        if (i_estado == 3) { // todos activos e inactivos
            if (seleccion == 0) {//todos
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0 and p.tab_est<> 0 order by p.tab_id";
            } else if (seleccion == 1) {
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0  and p.tab_est<> 0 and p.tab_id like '" + consulta + "' order by p.tab_id";
            } else {
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0  and p.tab_est<> 0 and p.tab_subdes like '" + consulta + "' order by p.tab_id";
            }
        } else {
            if (seleccion == 0) {//todos
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0 and p.tab_est=" + i_estado + "  order by p.tab_id";
            } else if (seleccion == 1) {
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0 and p.tab_est=" + i_estado + " and p.tab_id like '" + consulta + "' order by p.tab_id";
            } else {
                SQL_CATEGORIA = "select p.tab_id,p.tab_subdes,p.tab_subdir,p.tab_est,p.tab_nomrep,p.tab_ord, p.tab_usuadd, p.tab_fecadd, p.tab_usumod, p.tab_fecmod from ttabgen p "
                        + "where p.tab_cod=33 and p.tab_id<>0 and p.tab_est=" + i_estado + " and p.tab_subdes like '" + consulta + "' order by p.tab_id";
            }
        }
        P_WHERE = SQL_CATEGORIA;
        try {
