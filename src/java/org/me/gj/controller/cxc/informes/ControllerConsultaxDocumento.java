...
public class ControllerConsultaxDocumento extends SelectorComposer<Component> implements InterfaceControllers {
    ...
    Textbox txt_tipid, txt_tipdes, txt_nrodoc, txt_cli_id, txt_cli_des, txt_nroiden,  txt_direcc, txt_nescab_id,
            txt_nroped, txt_nreg, txt_zon_id, txt_zon_des, txt_ven_id, txt_ven_des, txt_dep, txt_lp_id, txt_lp_des,
    ...
    //Variables publicas
    String s_nrodoc;
	int s_tipdoc, i_flag = 0;
	public static boolean bandera = false;
    //Variables de Session
    Session sesion = Sessions.getCurrent();
    UsuariosCredential objUsuCredential = (UsuariosCredential) sesion.getAttribute("usuariosCredential");
    private static final Logger LOGGER = Logger.getLogger(ControllerConsultaxDocumento.class);
    ...
    @Listen("onClick=#btn_observacion")
    public void botonObservacion(){
    	if(objlstVtasDet.getSize()>0){
	    	Map objMapObjetos = new HashMap();
	        String modoEjecucion = "ConsultaxDoc";
	        objMapObjetos.put("txt_nrodoc", txt_nrodoc);
	        objMapObjetos.put("validaBusqueda", modoEjecucion);
	        objMapObjetos.put("controlador", "ControllerConsultaxDocumento");
	        Window window = (Window) Executions.createComponents("/org/me/gj/view/componentes/LovGlosa.zul", null, objMapObjetos);
	        window.doModal();
        }else{
        	if(i_flag == 0){
		        Messagebox.show("Debe de ingresar valores numericos en el campo 'NRO DOC.' para hacer la observacion", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION, new EventListener<Event>() {
   ...
   @Listen("onBlur=#txt_nrodoc;onSelect=#cb_tipdoc")
    ...
    public double[] calcularTotales() {
        ...
        for (int i = 0; i < objlstVtasDet.getSize(); i++) {
            ...
            totales[3] = totales[3] + ((VtasDet) objlstVtasDet.get(i)).getVtas_impper(); 
        }
        return totales;
    }
    ...
    public void limpiarCampos() {
        ...
        txt_totvvta.setValue(null);
        txt_igv.setValue(null);
        txt_totventa.setValue(null);
        txt_totimper.setValue(null);
    }
