...
public class ControllerAnuRefAcredDocVen extends SelectorComposer<Component> implements InterfaceControllers {

    //Componentes Web
    @Wire
    Tab tab_listaAnuRefAcredDocVen, tab_mantenimientoAnuRefAcredDocVen;
    @Wire
    ...
    @Wire
    Listbox lst_anurefdocven, lst_docven;
    @Wire
    Button btn_consulta;
    @Wire
    ...
    public void busquedaRegistros() throws SQLException {
        ...
        if (cb_busq_periodo.getValue().equals(f_periodo) || f_emisioni.equals("")) {
            ...
            int tipdoc = (int) (cb_tipodoc.getSelectedIndex() == -1 ? 0 : Integer.parseInt(cb_tipodoc.getSelectedItem().getValue().toString().toString()));
            ...
            if (objlstctacobcab.getSize() > 0) {
                lst_anurefdocven.setModel(objlstctacobcab);
            } else {
    ...
    @Listen("onSelect=#lst_anurefdocven")
    public void seleccionaRegistro() throws SQLException {
        objCtaCobCab = (CtaCobCab) lst_anurefdocven.getSelectedItem().getValue();
        if (objCtaCobCab == null) {
            objCtaCobCab = objlstctacobcab.get(i_selCab + 1);
        }
        i_selCab = lst_anurefdocven.getSelectedIndex();
        limpiarCampos();
    ...
    public void lovSupervisor() {
        if (bandera == false) {
            ...
            if (txt_busq_supid.getValue().isEmpty()) {
                ...
                objMapObjetos.put("controlador", "ControllerAnuRefAcredDocVen");
