...
public class ControllerProcPedPend extends SelectorComposer implements InterfaceControllers {

    //Componentes Web
    @Wire
    Tab tab_listaPedPend, tab_mantenimientoPedPend;
    @Wire
    Toolbarbutton tbbtn_btn_nuevo, tbbtn_btn_editar, tbbtn_btn_eliminar, tbbtn_btn_guardar, tbbtn_btn_deshacer,
            tbbtn_btn_buscar, tbbtn_btn_nuevopro, tbbtn_btn_editarpro, tbbtn_btn_eliminarpro,
            tbbtn_btn_guardarpro, tbbtn_btn_deshacerpro, tbbtn_filtro_buscar, tbbtn_cxc_cli, tbbtn_liberar, tbbtn_revisado;
    @Wire
    Combobox cb_periodo, cb_situacion, cb_simbmon, cb_tipoventa, cb_pronpag;
    @Wire
    Datebox d_filtro_fecha, d_fecemi, d_fecadd, d_fecmod;
    @Wire
    Doublebox db_por_lispre, db_por_convta, db_tipcam, db_det_sumtotal,
            ...
            db_pednofac, db_totalnofac, db_limcred, db_saldo, db_det_totitem, db_saldocred;
    @Wire
    Textbox txt_filtro_codsuper, txt_filtro_dessuper, txt_filtro_codmotivo, txt_filtro_desmotivo, txt_filtro_codven, txt_filtro_nomven, //----->tab_lista
            txt_nropedven, txt_situacion, txt_dni, txt_ruc,
            txt_proid, txt_cli_id, txt_cli_razsoc, /*txt_limcred, txt_limdoc, txt_saldo,*/ txt_estado, txt_clidir_direcc,
            ...
            txt_listaprecio, txt_formapago, txt_det_usuadd, txt_est;
    @Wire
    Listbox lst_pedidopendiente, lst_pedven_detalle, lst_observacliente;
    @Wire
    Button btn_consultarpedven, btn_recped, btn_ordenasc, btn_ordendesc;
    @Wire
    Intbox txt_pedvendet_key, txt_limdoc, txt_frac, txt_facbol/*(oculto)*/, txt_saldodoc;
    @Wire
    Longbox txt_clidir_id;
    @Wire
    Checkbox chk_ingresado, chk_estado, chk_revisado,//----->busqueda
    ...
    private static final Logger LOGGER = Logger.getLogger(ControllerProcPedPend.class);
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        objDaoCondicion = new DaoCondicion();
    ...
    public void VerificarTransacciones() throws SQLException {
        ...
        objAccesos = objDaoAccesos.Verifica_Permisos_Transacciones(40205000, usuario, empresa, sucursal);
    ...
    public void busquedaRegistrosFecha() throws SQLException {
        busquedaRegistros(1);
    }

    @Listen("onClick=#btn_ordenasc")
    public void busquedaRegistrosMontoAsc() throws SQLException {
        busquedaRegistros(2);
    }

    @Listen("onClick=#btn_ordendesc")
    public void busquedaRegistrosMontoDesc() throws SQLException {
        busquedaRegistros(3);
    }

    public void busquedaRegistros(int orden) throws SQLException {
        limpiarLista();
        limpiarListaObservacion();
        ...
        if (cb_periodo.getValue().equals(f_periodo) || f_emisioni.equals("")) {
            ...
            String s_supervisor = txt_filtro_codsuper.getValue().isEmpty() ? "" : txt_filtro_codsuper.getValue();
            String s_ven_id = txt_filtro_codven.getValue().isEmpty() ? "" : txt_filtro_codven.getValue();
            String s_motivo = txt_filtro_codmotivo.getValue().isEmpty() ? "" : txt_filtro_codmotivo.getValue();
            String situacion = cb_situacion.getSelectedItem().getValue().toString().equals("") ? "3,4" : cb_situacion.getSelectedItem().getValue().toString();
            int estado = chk_estado.isChecked() ? 1 : 2;

            objListaPedidoVentaCab = new ListModelList();
            objListaPedidoVentaCab = objDaoGenPedVen.busquedaPedidoPendiente(situacion, s_periodo, s_supervisor, s_motivo, f_emisioni, s_ven_id, estado, orden);
            //Validar la tabla sin registro
            if (objListaPedidoVentaCab.getSize() > 0) {
                lst_pedidopendiente.setModel(objListaPedidoVentaCab);
        ...
        limpiaAuditoria();
        txt_formapago.setStyle(null);
    }

    @Listen("onSelect=#lst_pedidopendiente")
    public void seleccionaRegistro() throws SQLException {
        limpiarCampos();
        limpiarCamposDetalle();
        limpiarCamposTotales();
        objPedidoVentaCab = (PedidoVentaCab) lst_pedidopendiente.getSelectedItem().getValue();
        if (objPedidoVentaCab == null) {
            objPedidoVentaCab = objListaPedidoVentaCab.get(i_selCab + 1);
        }
        i_selCab = lst_pedidopendiente.getSelectedIndex();
    ...
    public void botonEditar() throws SQLException {
        if (lst_pedidopendiente.getSelectedIndex() == -1) {
            Messagebox.show("Por favor seleccione un pedido de venta", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            /*if (objPedidoVentaCab.getPcab_situacion() != 1) {
             Messagebox.show("El pedido de venta ya no puede ser modificada", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
             } else {*/
            s_estado = "M";
            habilitaBotones(true, false);
            habilitaTab(true, false);
            ...
            Messagebox.show("Está seguro que desea eliminar el producto", "ERP-JIMVER", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener() {
                ...
                public void onEvent(Event event) throws Exception {
                    if (((Integer) event.getData()).intValue() == Messagebox.OK) {
                        ...
                        double data[];
                        data = calculosTotal();
                        db_det_sumtotal.setValue(data[0]);
                        db_det_totitem.setValue(data[1]);
                        lst_pedven_detalle.setModel(objListaPedidoVentaDet);
                    }
                }
            });
    ...
    @Listen("onClickAprobEspecial=#lst_pedidopendiente")
    public void AprobPedidoGlosa(ForwardEvent evt) throws SQLException {
        tbbtn_liberar = (Toolbarbutton) evt.getOrigin().getTarget();
        objlstAux = (ListModelList) lst_pedidopendiente.getModel();
        final Listitem item = (Listitem) tbbtn_liberar.getParent().getParent();
        int pos = item.getIndex();
        if (objlstAux.get(item.getIndex()).getPcab_estado() != 1) {
            Messagebox.show("El pedido ya esta REVISADO, no puede activarlo", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            nroped = objlstAux.get(pos).getPcab_nroped();
            s_mensaje = "Confirme la autorización para activar el pedido Nro:" + nroped;
            Messagebox.show(s_mensaje, "ERP-JIMVER", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener() {
                @Override
                public void onEvent(Event event) throws Exception {
                    if (((Integer) event.getData()).intValue() == Messagebox.OK) {
                        ParametrosSalida objParamCabecera;
                        String nropedido = objlstAux.get(item.getIndex()).getPcab_nroped();
                        String motrec = objlstAux.get(item.getIndex()).getPcab_motrec();
                        objParamCabecera = objDaoGenPedVen.activarAprobEspecial(nropedido, motrec);
                        if (objParamCabecera.getFlagIndicador() == 0) {
                            limpiarLista();
                            busquedaRegistros(1);
                        }
    ...
    @Listen("onClickCliente=#lst_pedidopendiente")
    public void goManCliente(ForwardEvent evt) {
        obtieneClienteLista(evt, "/mantenimiento/ManCliente.zul");
    }

    @Listen("onClickCxC=#lst_pedidopendiente")
    public void goCliente_CXC(ForwardEvent evt) {
        obtieneClienteLista(evt, "/informes/ConsultaxCliente.zul");
    }

    public void obtieneClienteLista(ForwardEvent evt, String zul) {
        tbbtn_cxc_cli = (Toolbarbutton) evt.getOrigin().getTarget();
        objlstAux = (ListModelList) lst_pedidopendiente.getModel();
        Listitem item = (Listitem) tbbtn_cxc_cli.getParent().getParent();

        controlador = "ControllerProcPedPend";
        s_cliid = objlstAux.get(item.getIndex()).getCli_id();
        Executions.sendRedirect("/org/me/gj/view/cxc" + zul);
    }

    @Listen("onClickRevisado=#lst_pedidopendiente")
    public void actualizarSituRevisado(ForwardEvent evt) throws SQLException {
        tbbtn_revisado = (Toolbarbutton) evt.getOrigin().getTarget();
        objlstAux = (ListModelList) lst_pedidopendiente.getModel();
        final Listitem item = (Listitem) tbbtn_revisado.getParent().getParent();
        //ParametrosSalida objParamCabecera = new ParametrosSalida();
        int pos = item.getIndex();
        // boolean valida = true;

        if (objlstAux.get(item.getIndex()).getPcab_estado() != 1) {
            Messagebox.show("El pedido se encuentra anulado, no se puede activar", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            nroped = objlstAux.get(pos).getPcab_nroped();
            if (objlstAux.get(item.getIndex()).getPcab_situacion() == 4) {
                Messagebox.show("El pedido ya fue revisado", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
            } else {
                Messagebox.show("Confirme la Autorizacion para marcar como 'REVISADO' el pedido N° " + nroped, "ERP-JIMVER", Messagebox.OK | Messagebox.CANCEL, Messagebox.EXCLAMATION, new EventListener() {
                    @Override
                    public void onEvent(Event event) throws Exception {
                        if (((Integer) event.getData()).intValue() == Messagebox.OK) {
                            ParametrosSalida objParamCabecera;
                            //objlstAux.get(item.getIndex()).setValSelec(chk_Reg.isChecked());
                            //lst_pedidopendiente.setModel(objlstAux);
                            //objPedidoVentaCab.setPcab_nroped(objlstAux.get(item.getIndex()).getPcab_nroped());
                            String nropedido = objlstAux.get(item.getIndex()).getPcab_nroped();
                            //String lp_id = objlstAux.get(item.getIndex()).getLp_id();
                            //objPedidoVentaCab.setEmp_id(emp_id);
                            //objPedidoVentaCab.setSuc_id(suc_id);
                            objParamCabecera = objDaoGenPedVen.actualizarSituRevisado(nropedido);
                            if (objParamCabecera.getFlagIndicador() == 0) {
                                limpiarLista();
                                busquedaRegistros(1);
                            }
    ...
    @Listen("onClickLiberar=#lst_pedidopendiente")
    public void liberarPedido(ForwardEvent evt) throws SQLException {
        /*final Checkbox chk_Reg = (Checkbox) evt.getOrigin().getTarget();
         final ListModelList<PedidoVentaCab> objlstAux = (ListModelList) lst_pedidopendiente.getModel();
         final Listitem item = (Listitem) chk_Reg.getParent().getParent();*/

        tbbtn_liberar = (Toolbarbutton) evt.getOrigin().getTarget();
        objlstAux = (ListModelList) lst_pedidopendiente.getModel();
        final Listitem item = (Listitem) tbbtn_liberar.getParent().getParent();
        /*if (objlstAux.get(item.getIndex()).getPcab_estado() == 2 || objListaPedidoVentaCab.get(item.getIndex()).getPcab_situacion() == 4 || objListaPedidoVentaCab.get(item.getIndex()).getPcab_situacion() == 2 || objListaPedidoVentaCab.get(item.getIndex()).getPcab_situacion() == 1) {
         Messagebox.show("El Pedido de venta ya no puede ser Activado", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
         } else {*/
        int pos = item.getIndex();
        if (objlstAux.get(item.getIndex()).getPcab_estado() != 1) {
            Messagebox.show("El pedido se encuentra anulado, no se puede activar", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
            ...
            Messagebox.show(s_mensaje, "ERP-JIMVER", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener() {
                ...
                public void onEvent(Event event) throws Exception {
                    if (((Integer) event.getData()).intValue() == Messagebox.OK) {
                        ...
                        //objPedidoVentaCab.setEmp_id(emp_id);
                        //objPedidoVentaCab.setSuc_id(suc_id);
                        objParamCabecera = objDaoGenPedVen.activarPedidoVenta(nropedido);

                        if (objParamCabecera.getFlagIndicador() == 0) {
                            limpiarLista();
                            busquedaRegistros(1);
                        }
                        /*Messagebox.show(objParamCabecera.getMsgValidacion(), "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION), new EventListener() {
                         @Override
                         public void onEvent(Event event) throws Exception {
                         if (((Integer) event.getData()).intValue() == Messagebox.OK) {
    ...
    public void validaSupervisor() throws SQLException {
        if (!txt_filtro_codsuper.getValue().isEmpty()) {
            ...
            txt_filtro_dessuper.setText("");
        }
        bandera = false;
    }

    @Listen("onOK=#txt_filtro_codmotivo")
    public void lovMotivoRechazo() {
        if (bandera == false) {
            bandera = true;
            if (txt_filtro_codmotivo.getValue().isEmpty()) {
                modoEjecucion = "ProcPedPend";
                Map objMapObjetos = new HashMap();
                objMapObjetos.put("txt_idmotivo", txt_filtro_codmotivo);
                objMapObjetos.put("txt_desmotivo", txt_filtro_desmotivo);
                objMapObjetos.put("validaBusqueda", modoEjecucion);
                ...
                d_filtro_fecha.select();
            }
        }
    }

    @Listen("onBlur=#txt_filtro_codmotivo")
    public void validaMotivoRechazo() throws SQLException {
        if (!txt_filtro_codmotivo.getValue().isEmpty()) {
            /*if (!txt_filtro_codmotivo.getValue().matches("[0-9]*")) {
             Messagebox.show("Por favor ingrese valores numéricos", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION, new EventListener() {
             @Override
             public void onEvent(Event event) throws Exception {
             if (((Integer) event.getData()).intValue() == Messagebox.OK) {
             txt_filtro_codmotivo.setValue("");
             txt_filtro_desmotivo.setValue("");
             txt_filtro_codmotivo.focus();
             }
             }
             });
            ...
            String motid = txt_filtro_codmotivo.getValue().toUpperCase();
            ...
            if (objMotRec == null) {
                ...
                LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | ha cargado los datos del motivo de rechazo con codigo " + motid + " y ha encontrado 1 registro(s)");
                //txt_filtro_codmotivo.setValue(Utilitarios.lpad(String.valueOf(objMotRec.getMrec_id()), 3, "0"));
                txt_filtro_codmotivo.setValue(String.valueOf(objMotRec.getMrec_id()));
                txt_filtro_desmotivo.setValue(objMotRec.getMrec_des());
            }
            //}
        } else {
            txt_filtro_desmotivo.setValue("");
        ...
        if (!txt_cli_id.getValue().isEmpty()) {
            if (!txt_cli_id.getValue().matches("[0-9]*")) {
                ...
                if (objCliente == null /*|| objClientes == null*/) {
                    ...
                    //db_limcred.setValue(objCliente.getCli_limcredemp());
                    //txt_limdoc.setValue(objCliente.getCli_limdocemp());
                    ...
                    txt_estado.setValue(objCliente.getCli_est() == 1 ? "ACTIVO" : "ANULADO");
        ...
        if (!txt_id_condven.getValue().isEmpty()) {
            if (!txt_id_condven.getValue().matches("[0-9]*")) {
                ...
                if (objCondicion == null) {
                    ...
                    if (s_estado.equals("M") && objCliente != null) {
            ...
            if (!txt_proid.getValue().matches("[0-9]*")) {
                ...
                if (objProductos == null) {
                    ...
                    if (objPreciosVenta != null) {
                        ...
                        txt_det_idlispre.setValue(objPreciosVenta.getLp_id());
                        txt_det_deslispre.setValue(objPreciosVenta.getLp_des());
    ...
    @Listen("onClick=#btn_difprecio")
    public void VerDiferenciaPrecio() throws SQLException {
        if (lst_pedidopendiente.getSelectedIndex() == -1) {
            Messagebox.show("Seleccione Pedido de venta", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            //obtenemos el index 
            int index = lst_pedidopendiente.getSelectedIndex();
            String nropedven = objListaPedidoVentaCab.get(index).getPcab_nroped();
            Map objMapObjetos = new HashMap();
            modoEjecucion = "GenPedVen";
            objMapObjetos.put("nropedven", nropedven);
            objMapObjetos.put("validaBusqueda", modoEjecucion);
            objMapObjetos.put("controlador", "ControllerGenPedVen");
            Window window = (Window) Executions.createComponents("/org/me/gj/view/componentes/LovDiferenciaPrecio.zul", null, objMapObjetos);
            window.doModal();
        }
    }
    ...
    public Object generaRegistroCabecera() throws SQLException {
        ...
        double pcab_salcre = db_saldo.getValue();
    ...
    public void llenarCampos() {
        try {
            ...
            //objCliFin = objDaoCliente.getClienteFin(objCliente.getCli_id().toString(), emp_id, suc_id);
            objCliFin = objDaoCtaCob.listaFinancieroCtaCob(Long.parseLong(objCliente.getCli_id()));
            objCondicion = objDaoCondicion.buscarCondicion(Integer.parseInt(objPedidoVentaCab.getCon_id()), "V");
        ...
        if (objPedidoVentaCab.getPcab_motrec_des() != null) {
            ...
            txt_situacion.setValue(objPedidoVentaCab.getPcab_situacion_des() + " (" + objMotivoRechazo.getMrec_id() + "-" + objMotivoRechazo.getMrec_des() + ")");
        } else {
            txt_situacion.setValue(objPedidoVentaCab.getPcab_situacion_des());
        }
        ...
        //db_limcred.setValue(objPedidoVentaCab.getPcab_limcre());
        db_limcred.setValue(objCliFin == null ? 0.0 : objCliFin.getClifin_limcred());
        txt_limdoc.setValue(objCliFin == null ? 0 : objCliFin.getClifin_limdoc());
        //txt_limdoc.setValue(String.valueOf(objPedidoVentaCab.getPcab_limdoc()));
        db_saldo.setValue(0.0);
        txt_estado.setValue(objPedidoVentaCab.getPcab_estado() == 1 ? "ACTIVO" : "ANULADO");
        ...
        //txt_id_condven.setValue(Utilitarios.lpad(String.valueOf(objCondicion.getConId()), 3, "0"));
        txt_id_condven.setValue(objCondicion.getConId());
        txt_des_condven.setValue(objCondicion.getConDes());
        ...
        txt_listaprecio.setValue(objPedidoVentaCab.getLp_des());
        txt_formapago.setValue(objPedidoVentaCab.getCond_des());
        if (txt_formapago.getValue().equals("CASH")) {
            txt_formapago.setStyle("font-size: 14px; left:50px; background-color:#F78181; color:#800040;");
        } else {
            txt_formapago.setStyle("font-size: 14px; left:50px; color:#800040;");
        }
        db_saldocred.setValue(objCliFin == null ? 0.0 : objCliFin.getClifin_saldo());
        txt_saldodoc.setValue(objCliFin == null ? 0 : objCliFin.getClifin_docemi());

        txt_est.setValue(objPedidoVentaCab.getPcab_estado() == 1 ? "ACTIVO" : "ANULADO");
        if (txt_est.getValue().equals("ANULADO")) {
            txt_est.setStyle("font-size: 14px; left:50px; background-color:#D8F5E0;");
        } else {
            txt_est.setStyle("font-size: 14px; left:50px;");
        }
    }

    public void llenarObservacion() throws SQLException {
        String nrocliente = objPedidoVentaCab.getCli_id();
        objlstCliObserva = null;
        objlstCliObserva = objDaoCtaCob.listaObservacionCliente(emp_id, suc_id, nrocliente);
        lst_observacliente.setModel(objlstCliObserva);
    }

    public void llenarValores() throws SQLException {
        String nrocliente = objPedidoVentaCab.getCli_id();
        objlstCliObserva = null;
        objlstCliObserva = objDaoCtaCob.listaObservacionCliente(emp_id, suc_id, nrocliente);
        lst_observacliente.setModel(objlstCliObserva);
    ...
    public void llenarCamposDetalle() throws SQLException {
        ...
        objListaPedidoVentaDet = null;
        objListaPedidoVentaDet = objDaoGenPedVen.listaPedidoVentaDet(emp_id, suc_id, nropedkey);
        lst_pedven_detalle.setModel(objListaPedidoVentaDet);
    }
    ...
    public void seleccionaTab(boolean b_valida1, boolean b_valida2) {
        tab_listaPedPend.setSelected(b_valida1);
        tab_mantenimientoPedPend.setSelected(b_valida2);
    }

    public void habilitaTab(boolean b_valida1, boolean b_valida2) {
        tab_listaPedPend.setDisabled(b_valida1);
        tab_mantenimientoPedPend.setDisabled(b_valida2);
    }
    ...
    public void limpiarCampos() {
        ...
        //-----------//
        txt_listaprecio.setValue("");
        txt_formapago.setValue("");
        txt_est.setValue("");
        db_saldocred.setValue(null);
        txt_saldodoc.setValue(null);
    ...
    public double[] calculosTotalLista() {
        ...
        for (i = 0; i < objListaPedidoVentaCab.getSize(); i++) {
            //if (objListaPedidoVentaCab.get(i).getPcab_situacion() == 3) {
            sumRec++;
    ...
    public void llenarCamposTotalesLista() {
        ...
        db_pednofac.setValue(data[0]);
        db_totalnofac.setValue(data[1]);
    ...
    public String validaFraccionVenta() throws SQLException {
        ...
        if (txt_frac.getValue() == frac1 || txt_frac.getValue() == frac2 || txt_frac.getValue() == 0) {
            msg = "";
        } else {
    ...
    public void limpiarCamposTotales() {
        db_det_sumtotal.setValue(null);
        db_det_totitem.setValue(null);
    }

    public void limpiarCamposTotalesLista() {
        db_pednofac.setValue(null);
        db_totalnofac.setValue(null);
    }
    ...
    public void llenarCamposTotales() {
        ...
        db_det_sumtotal.setValue(data[0]);
        db_det_totitem.setValue(data[1]);
        txt_det_usuadd.setValue(objPedidoVentaCab.getPcab_usuadd());
    }
    ...
    public void limpiarListaObservacion() {
        objlstCliObserva = null;
