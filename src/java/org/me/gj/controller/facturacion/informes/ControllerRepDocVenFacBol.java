...
public class ControllerRepDocVenFacBol extends SelectorComposer<Component> implements InterfaceControllers {
    ...
    @Wire
    Combobox cb_busq_periodo, cb_tipoventa, cb_busq_tipodoc, cb_busq_serie;
    @Wire
    Datebox d_busq_fecha, d_fecemi, d_fecadd, d_fecmod, d_fecdesp;

    @Wire
    Doublebox db_limcred, txt_cant, txt_total, db_punit, db_valvta, db_pordes, db_totdoc, db_valdes, db_porigv, db_valigv, db_total;
    ...
    Textbox txt_busq_supid, txt_busq_supdes, txt_busq_venid, txt_busq_vendes, txt_lstvenid, txt_lstvendes, txt_lstsupid, txt_lstsupdes, txt_usuadd, txt_usumod,
            txt_docdes, txt_nrodoc,  txt_cli_id, txt_cli_razsoc, txt_ven_id, txt_ven_des, txt_nroped,
    ...
    @Wire
    Listbox lst_repdocven, lst_docven;
    @Wire
    Button btn_consulta;
    @Wire
    ...
    private static final Logger LOGGER = Logger.getLogger(ControllerRepDocVenFacBol.class);
    ...
    @Listen("onCreate=#tabbox_repdocven")
    public void llenaRegistros() throws SQLException {
        ...
        cb_busq_tipodoc.setModel(objlstTipDoc);
    ...
    public void VerificarTransacciones() throws SQLException {
        ...
        objAccesos = objDaoAccesos.Verifica_Permisos_Transacciones(40401010, usuario, empresa, sucursal);
    ...
    public void busquedaRegistros() throws SQLException {
        ...
        if (cb_busq_periodo.getValue().equals(f_periodo) || f_emisioni.equals("")) {
            ...
            int tipdoc = (int) (cb_busq_tipodoc.getSelectedIndex() == -1 ? 0 : Integer.parseInt(cb_busq_tipodoc.getSelectedItem().getValue().toString()));
            String serie = cb_busq_serie.getValue().isEmpty() ? "" : Utilitarios.lpad(cb_busq_serie.getSelectedItem().getValue().toString(), 3, "0");

            objlstctacobcab = objDaoCtaCob.BusquedaFacturaBoletaCtaCobxTipoDocSerie(periodo, f_emisioni, sup_id, ven_id, tipdoc, serie);

            //Validar la tabla sin registro
            if (objlstctacobcab.getSize() > 0) {
                lst_repdocven.setModel(objlstctacobcab);
            } else {
    ...
    public void seleccionaRegistro() throws SQLException {
        objCtaCobCab = (CtaCobCab) lst_repdocven.getSelectedItem().getValue();
    ...
    @Listen("onSelect=#cb_busq_tipodoc")
    public void seleccionaTipoDocumento() throws SQLException {
        limpiarlistaCombo();
        if (cb_busq_tipodoc.getSelectedItem().getValue() != null) {
            int tipodoc = cb_busq_tipodoc.getSelectedItem().getValue();
            objlstNumeFac = objDaoNumeFac.listaSeriexNumeFac(tipodoc);
            if (!objlstNumeFac.isEmpty()) {
                cb_busq_serie.setDisabled(false);
                cb_busq_serie.setModel(objlstNumeFac);
            } else {
                cb_busq_serie.setDisabled(true);
            }
        }
    }
    ...
    @Listen("onSeleccion=#lst_repdocven")
    public void seleccionRegistroImprimir(ForwardEvent evt) {
        ...
        objlstctacobcab.get(item.getIndex()).setValSelec(chk_Reg.isChecked());
        lst_repdocven.setModel(objlstctacobcab);
    }

    @Listen("onCheck=#chk_imprimir")
    public void seleccionMultiple() {
        if (objlstctacobcab == null || objlstctacobcab.isEmpty()) {
            ...
            for (int i = 0; i < objlstctacobcab.getSize(); i++) {
                objlstctacobcab.get(i).setValSelec(chk_imprimir.isChecked());
            }
            lst_repdocven.setModel(objlstctacobcab);
        }
    }
    ...
    public void botonImprimir() throws SQLException {
        if (objlstctacobcab == null || objlstctacobcab.isEmpty()) {
            Messagebox.show("No hay registro Documento Venta Factura / Boleta para imprimir", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            String nrodoc = "";
            int i = 0;
            for (int j = 0; j < objlstctacobcab.getSize(); j++) {
                if (objlstctacobcab.get(j).isValSelec()) {
                    i = i + 1;
                    nrodoc += "'" + objlstctacobcab.get(j).getCtacob_nrodoc() + "',";
                }
            }
            if (i <= 0){
                Messagebox.show("Debe seleccionar un registro", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
            } else {
                nrodoc = "(" + nrodoc.substring(0, nrodoc.length() - 1) + ")";
                Map objMapObjetos = new HashMap();
                ...
                objMapObjetos.put("nrodoc", nrodoc);
                objMapObjetos.put("REPORT_LOCALE", new Locale("en", "US"));
                Window window = (Window) Executions.createComponents("/org/me/gj/view/impresion/facturacion/informes/LovImpresionDocVtaFacBol.zul", null, objMapObjetos);
                window.doModal();
            }
        }
    }
    ...
    public void llenarCampos(CtaCobCab objCtaCobCab) throws SQLException {
        ...
        d_fecemi.setValue(objCtaCobCab.getCtacob_fecemi());
        d_fecdesp.setValue(objCtaCobCab.getCtacob_fecent());
