...
public class DaoRegeneraCosto {
    ...
    private static final Logger LOGGER = Logger.getLogger(DaoRegeneraCosto.class);

    //Eventos Primarios - Transaccionales
    public ParametrosSalida regenerarCosto(int emp_id, int suc_id, String per_id, String stk_usuadd, String cos_rep) throws SQLException {
        ...
        try {
            ...
            if (s_msg.equals("N")) {
                ...
                s_msg = "";
                cst = null;
                String SQL_REGENERAR_COSTOS = "{call pack_tcostos.p_regenerar_costos(?,?,?,?,?,?,?,?,?)}";
                try {
                    con = new ConectaBD().conectar();
                    cst = con.prepareCall(SQL_REGENERAR_COSTOS);
                    cst.clearParameters();
                    cst.setInt("n_emp_id", emp_id);
                    cst.setInt("n_suc_id", suc_id);
                    cst.setString("c_per_id", per_id);
                    cst.setString("c_prod_ini", "");
                    cst.setString("c_prod_fin", "");
                    cst.setString("n_cos_rep", cos_rep);
                    ...
                    s_msg = cst.getString("c_msg");
                    i_flagErrorBD = cst.getInt("n_Flag");
                    LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | Regenero los Costos del Periodo " + per_id);
