...
public class ControllerPedComProd extends SelectorComposer<Component> {
    ...
    @Wire
    Listbox lst_pedcompcabxprod, lst_pedcompcab;
    @Wire
    ...
    ListModelList<PedidoComProd> objlstPedidoComProd;
    ...
    private static final Logger LOGGER = Logger.getLogger(ControllerPedComProd.class);
    ...
    public void VerificarTransacciones() throws SQLException {
        ...
        objAccesos = objDaoAccesos.Verifica_Permisos_Transacciones(10304020, usuario, empresa, sucursal);
        if (objUsuCredential.getRol() == 1) {
            LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | ha ingresado al Reporte de Pedido de Compra por Producto con el rol: ADMINISTRADOR");
        } else {
            LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | ha Ingresado al Reporte de Pedido de Compra por Producto con el rol: USUARIO NORMAL");
        }
        if (objAccesos.getAcc_imp() == 1) {
            tbbtn_btn_imprimir.setDisabled(false);
            LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | tiene acceso a impresion de la lista de Pedido de Compra por Producto");
        } else {
            tbbtn_btn_imprimir.setDisabled(true);
            LOGGER.info("[" + objUsuCredential.getComputerName() + "] | " + objUsuCredential.getCuenta() + " | no tiene acceso a impresion de la lista de Pedido de Compra por Producto");
        }
    }
    ...
    public void busquedaRegistros() throws SQLException {
        ...
        if (resultado.equals("F1>")) {
            ...
            lst_pedcompcabxprod.setSelectedIndex(-1);
            ...
            String prod_key = txt_prodid.getValue().isEmpty() ? "%%" : String.valueOf(Long.parseLong(txt_prodid.getValue()));
            String prod_id = txt_prodid.getValue().isEmpty() ? "%%" : txt_prodid.getValue();
            objlstPedidoComProd = new ListModelList<PedidoComProd>();
            objlstPedidoComProd = objDaoPedCom.listaTotalxProducto(emp_id, suc_id, per, "2", f_emisioni, f_emisionf, prov_id, prod_id, prod_key);
            if (objlstPedidoComProd.getSize() > 0) {
                lst_pedcompcabxprod.setModel(objlstPedidoComProd);
                llenarCamposTotales();
            } else {
                objlstPedidoComProd = null;
                lst_pedcompcabxprod.setModel(objlstPedidoComProd);
                limpiarCamposTotales();
    ...
    @Listen("onSelect=#lst_pedcompcabxprod")
    public void seleccionaRegistroTotal() throws SQLException {
        objPedidoComProd = (PedidoComProd) lst_pedcompcabxprod.getSelectedItem().getValue();
        if (objPedidoComProd == null) {
            objPedidoComProd = objlstPedidoComProd.get(i_selCab + 1);
        }
        i_selCab = lst_pedcompcabxprod.getSelectedIndex();
        String per = objPedidoComProd.getPeriodo();
        String fecini = objPedidoComProd.getFecemi();
        String fecfinal = objPedidoComProd.getFecfin();
        String pro_id = objPedidoComProd.getPro_id();
        objlstPedidoCompraCab = null;
        objlstPedidoCompraCab = objDaoPedCom.PedidoCompraxProducto(emp_id, suc_id, per, "2", fecini, fecfinal, pro_id);
        LimpiarCampos();
        lst_pedcompcab.setModel(objlstPedidoCompraCab);
    }
    ...
    public void botonImprimir() throws SQLException {
        if (objlstPedidoComProd == null || objlstPedidoComProd.isEmpty()) {
            Messagebox.show("No hay pedidos de compra por producto para imprimir", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            if (lst_pedcompcabxprod.getSelectedIndex() >= 0) {
                objPedidoComProd = (PedidoComProd) lst_pedcompcabxprod.getSelectedItem().getValue();
                if (objPedidoComProd == null) {
                    objPedidoComProd = objlstPedidoComProd.get(i_selCab + 1);
                }
                ...
                objMapObjetos.put("pro_id", objPedidoComProd.getPro_id());
                objMapObjetos.put("pro_des", objPedidoComProd.getPro_des());
                objMapObjetos.put("svimpto", objPedidoComProd.getSvimpto());
                objMapObjetos.put("svdesc", objPedidoComProd.getSvdesc());
                objMapObjetos.put("svafecto", objPedidoComProd.getSvafecto());
                objMapObjetos.put("REPORT_LOCALE", new Locale("en", "US"));
                Window window = (Window) Executions.createComponents("/org/me/gj/view/impresion/logistica/informes/LovImpresionPedComxProd.zul", null, objMapObjetos);
                window.doModal();
            } else {
                Messagebox.show("Seleccione un producto para imprimir sus pedidos");
            }
        }
    }
    ...
    public void lovProveedores() {
        if (bandera == false) {
            ...
            if (txt_provid.getValue().isEmpty()) {
                String tipo = "1";
                Map objMapObjetos = new HashMap();
                String modoEjecucion = "PedidoComProd";
                ...
                objMapObjetos.put("controlador", "ControllerPedidoComProd");
    ...
    public double[] calculosTotal() {
        ...
        for (i = 0; i < objlstPedidoComProd.getSize(); i++) {
            //data[0] = cont++;
            data[0] = data[0] + ((PedidoComProd) objlstPedidoComProd.get(i)).getCant();
            data[1] = data[1] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVdesc();
            data[2] = data[2] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVafecto();
            data[3] = data[3] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVinafecto();
            data[4] = data[4] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVexonerado();
            data[5] = data[5] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVimpto();
            data[6] = data[6] + ((PedidoComProd) objlstPedidoComProd.get(i)).getVtotal();
        }
        return data;
    }
