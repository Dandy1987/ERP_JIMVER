...
public class ControllerOrdComxProv extends SelectorComposer<Component> {
    ...
    @Wire
    Listbox lst_ordcompcabxprov, lst_ordcompcab;
    @Wire
    ...
    ListModelList<OrdCompProv> objlstOrdCompCabxProv;
    ...
    private static final Logger LOGGER = Logger.getLogger(ControllerOrdComxProv.class);
    ...
    public void VerificarTransacciones() throws SQLException {
        ...
        objAccesos = objDaoAccesos.Verifica_Permisos_Transacciones(10301010, usuario, empresa, sucursal);
    ...
    public void busquedaRegistros() throws SQLException {
        ...
        if (resultado.equals("F1>")) {
            ...
            lst_ordcompcabxprov.setSelectedIndex(-1);
            ...
            objlstOrdCompCabxProv = new ListModelList<OrdCompProv>();
            objlstOrdCompCabxProv = objDaoOrdCom.listaTotalxProveedor(emp_id, suc_id, per, "%45%", f_emisioni, f_emisionf, prov_id);
            if (objlstOrdCompCabxProv.getSize() > 0) {
                lst_ordcompcabxprov.setModel(objlstOrdCompCabxProv);
                llenarCamposTotales();
            } else {
                objlstOrdCompCabxProv = null;
                lst_ordcompcabxprov.setModel(objlstOrdCompCabxProv);
                limpiarCamposTotales();
    ...
    @Listen("onSelect=#lst_ordcompcabxprov")
    public void seleccionaRegistroTotal() throws SQLException {
        objOrdCompProv = (OrdCompProv) lst_ordcompcabxprov.getSelectedItem().getValue();
        String per = objOrdCompProv.getPeriodo();
        String fecini = objOrdCompProv.getFecemi();
        String fecfinal = objOrdCompProv.getFecfin();
        String prov_id = String.valueOf(Long.parseLong(objOrdCompProv.getProv_id()));
        objlstOrdCompCab = null;
        objlstOrdCompCab = objDaoOrdCom.OrdCompxProveedor(emp_id, suc_id, per, "%45%", fecini, fecfinal, prov_id);
        LimpiarCampos();
        lst_ordcompcab.setModel(objlstOrdCompCab);
    }
    ...
    public void botonImprimir() throws SQLException {
        if (objlstOrdCompCabxProv == null || objlstOrdCompCabxProv.isEmpty()) {
            Messagebox.show("No hay ordenes de compra por proveedor para imprimir", "ERP-JIMVER", Messagebox.OK, Messagebox.INFORMATION);
        } else {
            if (lst_ordcompcabxprov.getSelectedIndex() >= 0) {
                objOrdCompProv = (OrdCompProv) lst_ordcompcabxprov.getSelectedItem().getValue();
                if (objOrdCompProv == null) {
                    objOrdCompProv = objlstOrdCompCabxProv.get(i_selCab + 1);
                }
                Map objMapObjetos = new HashMap();
                objMapObjetos.put("empresa", objUsuCredential.getEmpresa());
                objMapObjetos.put("usuario", objUsuCredential.getCuenta());
                objMapObjetos.put("provid", objOrdCompProv.getProv_id());
                objMapObjetos.put("prov_razsoc", objOrdCompProv.getProv_razsoc());
                objMapObjetos.put("svimpto", objOrdCompProv.getSvimpto());
                objMapObjetos.put("svdesc", objOrdCompProv.getSvdesc());
                objMapObjetos.put("svafecto", objOrdCompProv.getSvafecto());
                objMapObjetos.put("REPORT_LOCALE", new Locale("en", "US"));
                Window window = (Window) Executions.createComponents("/org/me/gj/view/impresion/logistica/informes/LovImpresionOrdComxProv.zul", null, objMapObjetos);
                window.doModal();
            } else {
                Messagebox.show("Seleccione un proveedor para imprimir sus ordenes de compra");
            }
        }
    }
    ...
    public void lovProveedores() {
        if (bandera == false) {
            ...
            if (txt_provid.getValue().isEmpty()) {
                String tipo = "1";
                Map objMapObjetos = new HashMap();
                String modoEjecucion = "OrdComProv";
                ...
                objMapObjetos.put("controlador", "ControllerOrdCompxProv");
    ...
    public double[] calculosTotal() {
        ...
        for (i = 0; i < objlstOrdCompCabxProv.getSize(); i++) {
            //data[0] = cont++;
            data[0] = data[0] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getCant();
            data[1] = data[1] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVdesc();
            data[2] = data[2] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVafecto();
            data[3] = data[3] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVinafecto();
            data[4] = data[4] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVexonerado();
            data[5] = data[5] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVimpto();
            data[6] = data[6] + ((OrdCompProv) objlstOrdCompCabxProv.get(i)).getVtotal();
        }
        return data;
    }
